...
function AccordionItem({ className, ...props }: AccordionPrimitive.Item.Props) {
  return (
    <AccordionPrimitive.Item
      ...
      className={cn("not-last:border-b border-white/10", className)}
      {...props}
    />
  )
}
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      ...
      >
        {children}
        <ChevronDownIcon
  className="size-5 text-white/60 transition-transform duration-300 group-aria-expanded/accordion-trigger:rotate-180"
/>
      </AccordionPrimitive.Trigger>
